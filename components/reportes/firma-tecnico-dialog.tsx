...
interface FirmaTecnicoDialogProps {
  open: boolean;
...
export function FirmaTecnicoDialog({
  open,
  onOpenChange,
  reporteId,
  tecnicoNombre,
  onSuccess,
  capturaLocal = false,
}: FirmaTecnicoDialogProps) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [nombreFirmante, setNombreFirmante] = useState(tecnicoNombre);
  const signatureRef = useRef<SignatureCanvas>(null);
  const [isEmpty, setIsEmpty] = useState(true);

  useEffect(() => {
    if (open) {
      setNombreFirmante(tecnicoNombre);
      setIsEmpty(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/reportes/guardar-firma-tecnico", {
        method: "POST",
        headers: {
      ...
      onOpenChange(false);
      onSuccess(firmaData);
      alert("✅ Firma del técnico guardada exitosamente");
    } catch (error: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Firma Virtual del Técnico</DialogTitle>
          <DialogDescription>
            Firma el reporte técnico #{reporteId} como técnico responsable
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="nombreFirmante">Nombre del Técnico *</Label>
            <Input
              id="nombreFirmante"
