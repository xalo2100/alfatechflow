    ...
    try {
      ...
      if (!response.ok || !data.success) {
        setStatus({
          ...
          details: {
            error: data.error || "No se pudo conectar a Gemini",
          },
        });
      } else {
        setStatus({
      ...
      setStatus({
        ...
        details: {
          error: error.message || "No se pudo verificar la conexión con Gemini. Verifica que la API key esté configurada.",
        },
      });
    } finally {
      setChecking(false);
          ...
          <>
            ...
            <div className="space-y-2 text-sm flex-1">
              ...
              {status.details?.error && (
                <div className="bg-destructive/10 border border-destructive/20 rounded-md p-2 text-destructive text-xs mt-2 break-words whitespace-pre-wrap">
