...
interface ConfigResendDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess: () => void;
}

export function ConfigResendDialog({
  open,
  onOpenChange,
  onSuccess,
}: ConfigResendDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validación básica de formato (las API keys de Resend suelen empezar con "re_")
    if (!cleanedKey.startsWith("re_") && cleanedKey.length < 20) {
      setError("El formato de la API key no parece válido. Las API keys de Resend suelen empezar con 're_'");
      return;
    }
    ...
    try {
      ...
      const testResponse = await fetch("/api/resend/test-connection", {
        method: "POST",
      ...
      if (!testResponse.ok || !testData.success) {
        throw new Error(testData.error || "La API key no es válida. Verifica que sea correcta.");
      }
        ...
        .upsert(
          {
            ...
            descripcion: "API Key de Resend para envío de emails",
            creado_por: user.id,
          },
          {
            onConflict: "clave",
