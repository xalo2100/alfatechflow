...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!perfil || (perfil.rol !== "tecnico" && perfil.rol !== "admin")) {
      return NextResponse.json(
        { error: "Solo los técnicos y administradores pueden guardar firmas de técnico" },
        { status: 403 }
      );
    }
    ...
    // Agregar la firma del técnico
    reporteData.firma_tecnico = {
      imagen: firmaDataURL,
    ...
    return NextResponse.json({
      success: true,
      message: "Firma del técnico guardada exitosamente",
    });
  } catch (error: any) {
    console.error("Error en guardar-firma-tecnico API:", error);
    return NextResponse.json(
